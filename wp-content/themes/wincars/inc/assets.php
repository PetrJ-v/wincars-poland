...
function wincars_assets()
{
	...
	if (!is_page_template('service-pages/all-pdf.php') && !is_page_template('service-pages/watermark.php') && !(get_post_type() === 'pdf-price') && !(get_post_type() === 'collage')){
		...
		wp_enqueue_script('main', _TEMPLATEPATH . '/assets/js/main.js', ['jquery'], '1.0.2', ['in_footer' => true, 'strategy'  => 'async']);
	...
	if (is_front_page()) {
		...
		wp_enqueue_script('front-page', _TEMPLATEPATH . '/assets/js/front-page.js', array('wow', 'swiper'), '1.0.2', true);
	}
	if (is_single() && (get_post_type() != 'car')) {
		wp_enqueue_style('single-header', _TEMPLATEPATH . '/assets/css/first-screen/post-header.css', array('base'), '1.0.2');
		wp_enqueue_style('single', _TEMPLATEPATH . '/assets/css/single.css', array('base', 'single-header'), '1.0.2');
		wp_enqueue_style('swiper');
		wp_enqueue_script('swiper');
	...
	if (get_post_type() === 'car') {
		wp_enqueue_style('single-car', _TEMPLATEPATH . '/assets/css/templates/single-car.css', array('base'), '1.0.2');
		wp_enqueue_style('swiper');
		wp_enqueue_script('swiper');
		wp_enqueue_script('single-car', _TEMPLATEPATH . '/assets/js/single-car.js', array('jquery', 'swiper'), '1.0.0', true);
	}
	if (is_page_template('templates/about.php')) {
		wp_enqueue_style('about-page', _TEMPLATEPATH . '/assets/css/templates/about.css', array('base'), '1.0.2');
		wp_enqueue_script('about-page', _TEMPLATEPATH . '/assets/js/about.js', array('main'), '1.0.0', true);
	}
	if (is_page_template('templates/contacts.php')) {
		wp_enqueue_style('contacts-page', _TEMPLATEPATH . '/assets/css/templates/contacts.css', array('base'), '1.0.2');
		wp_enqueue_style('animate', _TEMPLATEPATH . '/assets/libs/wow/animate.min.css', array(), '3.5.1');
		wp_enqueue_script('wow', _TEMPLATEPATH . '/assets/libs/wow/wow.min.js', array('jquery'), '1.1.3', true);
	...
	if (is_page_template('templates/faq.php')) {
		wp_enqueue_style('faq-page', _TEMPLATEPATH . '/assets/css/templates/faq.css', array('base'), '1.0.2');
		wp_enqueue_style('animate', _TEMPLATEPATH . '/assets/libs/wow/animate.min.css', array(), '3.5.1');
		wp_enqueue_script('wow', _TEMPLATEPATH . '/assets/libs/wow/wow.min.js', array('jquery'), '1.1.3', true);
	...
	if (is_page_template('templates/page-feedbacks.php')) {
		wp_enqueue_style('testimonials', _TEMPLATEPATH . '/assets/css/templates/testimonials.css', array('base'), '1.0.2');
		wp_enqueue_style('animate', _TEMPLATEPATH . '/assets/libs/wow/animate.min.css', array(), '3.5.1');
		wp_enqueue_script('wow', _TEMPLATEPATH . '/assets/libs/wow/wow.min.js', array('jquery'), '1.1.3', true);
...
add_action( 'admin_enqueue_scripts', function(){
	wp_enqueue_style( 'my-wp-admin', _TEMPLATEPATH .'/assets/css/wp-admin.css', array(), '1.0.2' );
}, 99 );
